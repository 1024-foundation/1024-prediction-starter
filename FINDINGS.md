...
| 8 | **`/orderbook` & `/depth` are the WRONG endpoints for a 1024 order book.** They read the `prediction_orders` *resting* table, but the LP quotes *virtual / JIT* liquidity that never rests (it's materialized only at match time) — so they return a sparse, gappy, often one-sided book. Use **`/markets/:id/all-depths`**, which overlays the LP virtual ladder (the same data the first-party UI + WS show). | Fetch `/all-depths`; fall back to an indicative ladder only when it's empty. ([`data.ts`](src/lib/api/data.ts) `getAllDepths`) |
